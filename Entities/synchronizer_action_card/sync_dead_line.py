...
def sync_dead_line(policy_action: Action, e: EntitiesABC):
    ...
    def wrapper_get(action: Action):
        def wrapped(*args, **kwargs):
            ...
            if implementation_card is not None:
                action.set_dead_line_programmatically(implementation_card.dead_line_min())
    ...
    def wrapper_set(action: Action):
        def wrapped(*args, **kwargs):
            ...
            set_dead_line_recursively(action, e, new_dead_line, kwargs, actions_already_handled)
...
def set_dead_line_recursively(action: Action, e: EntitiesABC, new_dead_line, kwargs, already_handled: set):
    if (action is not None) and action.id not in already_handled:
        already_handled.add(action.id)
        synch_with_implementation_card(action, new_dead_line, e)
        action.set_dead_line_programmatically(new_dead_line)
        synch_with_higher_level_recursively(action, already_handled, new_dead_line, e, kwargs)


def synch_with_implementation_card(action: Action, new_dead_line, e: EntitiesABC):
    s: SynchronizerABC = e.synchronizer
    implementation_card = s.get_implementation_card(action.id)
    if implementation_card is not None:
...
def synch_with_higher_level_recursively(action: Action, already_handled: set, new_dead_line, e: EntitiesABC, kwargs):
    ...
    for card in cards:
        ...
        for each_parent_card in parent_cards:
            ...
            set_dead_line_recursively(ancestor_action, e, new_dead_line, kwargs, already_handled)
