...
def sync_dead_line(policy_action: Action, get_implementation_card: Callable[[str], Card], e: EntitiesABC):
    ...
    def wrapper_set(action: Action):
        def wrapped(*args, **kwargs):
            set_action_dead_line_recursively(action, get_implementation_card, e, s, args, kwargs)
...
def set_action_dead_line_recursively(action: Action, get_implementation_card: Callable, e: EntitiesABC,
                                     s: SynchronizerABC, args, kwargs):
    implementation_card = get_implementation_card(action.id)
    if implementation_card is not None:
    ...
    for card in cards:
        ...
        for each_parent_card in parent_cards:
            ancestor_action = s.card_has_policy_action(each_parent_card.id)
            set_action_dead_line_recursively(ancestor_action, get_implementation_card, e, s, args, kwargs)
