...
def present_card_list(e: EntitiesABC, p: PresentersABC, next_selection_index=None):
    my_cards = tuple(c for c in e.all_cards if c.owner == e.user)
    my_cards_names = tuple(c.name for c in e.all_cards if c.owner == e.user)
    my_cards_due_dates = tuple(c.due_date for c in e.all_cards if c.owner == e.user)
