...
def execute(from_: str, to_: str, feedback: Callable, e: EntitiesABC):
    ...
    def filter_action(action: Action):
        ...
        their_ball = action.get_owner().name != e.user.name
        if (datetime_in_question is not None) and (not action.is_done) and action_has_no_implementation and their_ball:
