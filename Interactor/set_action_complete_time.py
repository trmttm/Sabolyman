...
def execute(e: EntitiesABC, done_or_not, actions_indexes: Tuple[int, ...] = None):
    ...
    for i, action in enumerate(e.active_card.all_actions):
        if i in actions_indexes:
            if done_or_not and action.is_done:  # note that action mark done may be prevented if its policy action
                if action.time_completed is None:
                    action.set_completed_time(datetime.datetime.now())
            if not done_or_not and not action.is_done:  # just cuz done_or_not is False does not always set_imcomplete
